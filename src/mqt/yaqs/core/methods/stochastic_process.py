...
def circuit_calculate_stochastic_factor(state: MPS) -> NDArray[np.float64]:
    """Calculate the stochastic factor for a given state.
...
def circuit_create_probability_distribution(
    ...
    for site in range(n_sites):
        ...
        # --- 1-site jumps at this site ---
        for process in noise_model.processes:
            if len(process["sites"]) == 1 and process["sites"][0] == global_start + site:
                gamma = process["strength"]
                jump_operator = process["jump_operator"]
                print(f"DEBUG: Computing 1-site jump probability at site {site}, strength={gamma}")
        ...
        if site < n_sites - 1:
            for process in noise_model.processes:
                if len(process["sites"]) == 2 and process["sites"] == [global_start, global_start + 1]:
                    gamma = process["strength"]
                    jump_operator = process["jump_operator"]
                    print(f"DEBUG: Computing 2-site jump probability at sites [{site}, {site+1}], strength={gamma}")
...
def circuit_stochastic_process(
    state: MPS,
    noise_model: NoiseModel | None,
    dt: float,
    global_start: int,
    ...
    jump_dict = circuit_create_probability_distribution(state, noise_model, dt, global_start, sim_params)
    print(f"DEBUG: Global start: {global_start}")
